      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 lg:m-4 justify-center self-center">
        {posts?.map((post) => (
          <Link href={`blog/${post.slug}`} key={post?.title}>
            <div className="p-8 rounded-md shadow-md hover:shadow-xl glass w-full lg:w-[25em] transition-all duration-150 ease-linear relative z-20 overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 w-full h-1 bg-blue-400 z-10 blur-sm rounded-b-md animate-pulse" />
            </div>
          </Link>
        ))}
      </div>
